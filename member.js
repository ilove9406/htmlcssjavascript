...
function getID(){
    ...
    //alert(document.getElementByName("uid")); //[object NodeList]
    //alert(document.getElementByName("uid").value);
    //alert(document.memfrm.uid.value);//추천
...
function validate(){
    ...
    // alert(uname);
    // alert("#"+uname+"#");
    // alert("#"+uname.trim()+"#");//문자열 맨앞과 맨뒤 공백제거

    uname=uname.trim();
    if(uname.length<2){
        alert("이름 2글자이상 입력해주세요");
        document.getElementById("uname").focus();
        return false;
    }

    // 2) 아이디 글자갯수가 8~12사이 인지 확인
    ...
    if (upw.length < 5 || upw.length > 10) {
        alert("비밀번호는 5글자 이상, 10글자 이하여야 합니다.");
        document.getElementById("upw").focus();
        return false;
    }

    // 4) 주민번호 앞칸의 글자갯수가 6인지 숫자형인지 확인
    let jumin1 = document.getElementById("jumin1").value;
    if (jumin1.length !== 6 || isNaN(jumin1)) {
        alert("주민번호 앞 6자리를 숫자로 입력해주세요.");
